...
import { uploadImageToCloudinary } from "@/lib/cloudinary/upload-image-to-cloudinary";
...
export async function createEvent(_prevState: any, formData: FormData) {
  ...
  if (!parsedEvent.success) {
    return { message: parsedEvent.error.message };
  }
  ...
  if ((image as File).size > 0) {
    ...
    if (!parsedImage.success) {
      return { message: parsedImage.error.message };
    }
